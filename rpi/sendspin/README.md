...
`${HAL_DEVICE_NAME} Speaker`. Audio coexists with PAL TTS through the same
PulseAudio socket; ducking happens in Pulse so PAL doesn't have to know about
MA at runtime.
...
- PAL TTS streams (tagged `media.role=phone` via `PULSE_PROP`) trigger ducking.
- Sendspin streams (tagged `media.role=music`) duck by 25 dB while PAL speaks
...
> Music Assistant → Players → "*PAL Speaker*" → settings → **Channel Mode** = `Mono`
...
| Var | Default | Notes |
| --- | --- | --- |
| `HAL_DEVICE_NAME` | `PAL` | Player advertises as `${HAL_DEVICE_NAME} Speaker` |
...
in the *server* `.env`). When music is idle, they adjust PAL's TTS volume as
before.
...
the server. PAL will then call `media_player.media_pause` on the configured
