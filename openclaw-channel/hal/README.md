...
Voice → PAL STT → text
  ...
  runtime.channel.turn.runAssembled()  ← full agent pipeline w/ mcporter
          ↓
  POST /api/openclaw/response on PAL   ← delivery callback
          ↓
  text → TTS → speaker
  media_urls → orb display / QR codes
```

1. PAL sends transcribed voice text via HTTP POST to the plugin's
...
4. The agent's text response (and any media URLs) are delivered back
   to PAL via an HTTP callback to `/api/openclaw/response`.
5. PAL renders the text through TTS and displays any media on the orb.
...
Install the skill in the workspace so the agent knows how to use PAL:

```bash
cp openclaw-skill/hal/SKILL.md ~/.openclaw/<workspace>/skills/hal/SKILL.md
```

### 4. PAL server — Docker environment
...
PAL exposes its local tools as an MCP server at `/mcp` (SSE transport).
