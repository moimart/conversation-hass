...
8. **Web UI with themes** — A modern HAL 9000-inspired interface with metallic bezel ring, animated eye, live transcription, AI responses, and assistant state indicators. Six themes (`dark`, `sal`, `glados`, `birch`, `odyssey`, `japandi`); optional auto day/night switching driven by HA's `sun.sun` entity.
...
Six built-in themes, switchable from the web UI's theme picker, the LLM (`ui_set_theme` tool), the MQTT theme select, or the auto day/night scheduler:
