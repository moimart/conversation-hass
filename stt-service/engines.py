"""STT engines for the standalone stt-service: faster-whisper and NeMo/Nemotron.

This is the home of the heavy ASR engines and the self-healing inference base
(thread pool + timeout + model reload). It is owned entirely by the stt-service
image — the HAL AI server does NOT import this; it talks to this service over
HTTP via its own lightweight RemoteTranscriber (server/app/transcriber.py).

torch / faster_whisper / nemo are imported lazily inside the engine methods so
this module stays importable (e.g. in tests) without the multi-GB ASR stack.
"""
...
log = logging.getLogger("stt.engines")


class BaseTranscriber(ABC):
    """Abstract base for local STT engines.
    ...
    async def warm_up(self) -> None:
        ...
        request. Without this, the first transcribe call after a restart
        pays a 1-2 s setup tax. We pay it here at startup instead, where
        nobody is waiting."""
...
def create_transcriber(engine: str, model: str = "") -> BaseTranscriber:
    """
    Factory: create a local transcriber by engine name.
