...
PAL's hub UI is plug-in themable. A theme is a folder of files; drop it in, the server picks it up within ~10 seconds, and the hub's picker updates without a restart on either side. This document covers the contract, the available CSS variables, the optional effect.js API, and how to develop and ship a theme.
...
3. The server's theme registry picks it up on the next polling tick (default 10 s) and pushes a `themes_changed` event. The hub's dropdown updates; selecting your theme lazy-loads its CSS.
...
| `display_name` | yes | string | Shown in the hub picker and HA select. Free form (Unicode, em-dashes, etc.). |
...
A single CSS block scoped to `body.theme-<name>` that overrides any subset of the hub's CSS custom properties.
...
That's enough to recolor the orb's iris, glow, and ambient highlights. Everything else inherits from the hub's base `:root`.

### Best practice: copy `themes/dark/theme.css`

Use `themes/dark/theme.css` as a starter. It mirrors the hub `:root` defaults and gives you every variable in one place. Edit what you want and delete the rest — the hub's `:root` fills any variable you omit.
...
- `themes/matrix/theme.css` toggles the visibility of the `#matrix-rain` `<canvas>` (the canvas itself is part of the hub's base HTML).
...
The hub sets state classes on `<body>`: `state-idle`, `state-listening`, `state-processing`, `state-speaking`. Theme one state without affecting others:
...
These are the variables defined in the hub's `:root`. Override any in your theme; omit any you don't care about and the default applies.
...
Themes with `"effect": "<file>.js"` in their manifest provide a dynamic background or overlay. The hub imports the module *on first activation* and keeps the instance for the lifetime of the page; subsequent activations and deactivations call `start()` and `stop()` on the same controller.
...
export default function setup({ root }) {
    // `root` is the hub's <body>. You may mount canvases, divs,
...
- **Don't lean on existing DOM elements you didn't add.** The hub's structural HTML (orb, controls, transcript) is stable but treat it as someone else's UI. The `<canvas id="matrix-rain">` element is one published exception — see below.
...
The hub's base HTML reserves one element for theme effects:
...
5. The hub receives `themes_changed`, re-fetches `/api/themes`, and rebuilds its dropdown.
6. The first time someone selects your theme, the hub lazy-injects `<link rel="stylesheet" href="/themes/<name>/theme.css">` and (if your manifest declares one) `import("/themes/<name>/effect.js")`.

### What survives a hub refresh

The hub caches no theme assets — every CSS/JS file is served with `Cache-Control: no-cache`. Edit `theme.css`, refresh the hub, your changes appear.

### What survives a theme uninstall

If you remove your theme's directory while it's the active theme on a hub, the next `themes_changed` event will trigger a fallback to `dark` on every hub.
...
| Theme | Folder | Notes |
|---|---|---|
| dark | `themes/dark/` | Baseline PAL red. Mirrors the hub `:root`. |
...
The hub consumes this; you can hit it too for tooling or external installers.
...
**Q: Will my theme break across hub Chromium upgrades?**
